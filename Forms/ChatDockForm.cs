...
public class ChatDockForm : FormBase
{
    ...
    public ChatDockForm() : base(false, true)
    {
        ...
        VisibleChanged += (s, e) =>
        {
            ...
            {
                focusTimer.Start();
                // Apply theme-specific colors after the base class has applied general styling
                // This ensures our custom colors override the default ones
                this.BeginInvoke(new Action(() => UpdateThemeColors()));
            }
        };
    ...
    private void InputBox_KeyUp(object sender, KeyEventArgs e)
    {
        ...
        {
            var text = inputBox.Text.TrimEnd();
    ...
    private string ExtractFileNameFromBlock(string response, int codeBlockStart)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Error in ExtractFileNameFromBlock: {ex.Message}");
            return "";
        }
    }
    ...
    private string ExtractLanguageFromBlock(string response, int codeBlockStart, int codeStartEnd)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Error in ExtractLanguageFromBlock: {ex.Message}");
            return "";
        }
    }
    ...
    private string RemoveCodeBlocks(string response)
    {
        try
        {
            string cleaned = response;

            // Remove [CODE_BLOCK...]...[/CODE_BLOCK] blocks and their file_info tags including language tags
            while (true)
            {
                int codeStart = cleaned.IndexOf("[CODE_BLOCK");
                if (codeStart == -1) break;

                int codeStartEnd = cleaned.IndexOf("]", codeStart);
                if (codeStartEnd == -1) break;

                int codeEnd = cleaned.IndexOf("[/CODE_BLOCK]", codeStartEnd);
                if (codeEnd == -1) break;

                // Find the preceding file_info if it exists (search up to FileInfoSearchWindow chars before code start)
                int searchStart = Math.Max(0, codeStart - FileInfoSearchWindow);
                string searchArea = cleaned.Substring(searchStart, codeStart - searchStart);
                int fileInfoStart = searchArea.LastIndexOf("<file_info>");

                int removeStart;
                if (fileInfoStart != -1)
                {
                    removeStart = searchStart + fileInfoStart;
                }
                else
                {
                ...
                int removeEnd = codeEnd + "[/CODE_BLOCK]".Length;
                cleaned = cleaned.Remove(removeStart, removeEnd - removeStart);
            }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Error in RemoveCodeBlocks: {ex.Message}");
            return response;
        }
    }
}
