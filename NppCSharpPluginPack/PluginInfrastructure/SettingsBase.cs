...
public class SettingsBase
{
    ...
    private static void InvokeVersionInfoThread()
    {
        try
        {
            var thread = new Thread(() =>
            {
                ...
                {
                    Debug.WriteLine($"Error in {nameof(InvokeVersionInfoThread)}: {ex.Message}");
                }
            })
            {
                IsBackground = true
    ...
    public void ShowDialog(bool debug = false)
    {
        ...
        {
            Name = "SettingsForm",
            Text = $"Settings - {Main.PluginName} - a Plugin by Max Lachlan",
        ...
        FormStyle.ApplyStyle(dialog, true);
